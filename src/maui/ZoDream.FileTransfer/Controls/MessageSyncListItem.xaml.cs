...
public partial class MessageSyncListItem : ContentView
{
    ...
    public SyncMessageItem ItemSource {
        get { return (SyncMessageItem)GetValue(ItemSourceProperty); }
        set { SetValue(ItemSourceProperty, value); }
    }

    // Using a DependencyProperty as the backing store for ItemSource.  This enables animation, styling, binding, etc...
    public static readonly BindableProperty ItemSourceProperty =
        BindableProperty.Create(nameof(ItemSource), typeof(SyncMessageItem),
            typeof(MessageSyncListItem), null,
            propertyChanged: (d, _, o) => {
                (d as MessageSyncListItem)?.UpdateView();
            });
